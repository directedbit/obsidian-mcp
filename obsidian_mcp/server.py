...
async def read_note_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to read note: {str(e)}")
        )
...
async def create_note_tool(
    ...
    overwrite: Annotated[
        bool,
        Field(
            description="Whether to overwrite if the note already exists", default=False
        ),
    ] = False,
    ctx=None,
):
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to create note: {str(e)}")
        )
...
async def update_note_tool(
    ...
    create_if_not_exists: Annotated[
        bool, Field(description="Create the note if it doesn't exist", default=False)
    ] = False,
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to update note: {str(e)}")
        )
...
async def delete_note_tool(path: str, ctx=None):
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to delete note: {str(e)}")
        )
...
async def search_notes_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to search notes: {str(e)}")
        )
...
async def search_by_date_tool(
    date_type: Annotated[
        ...
        Field(description="Type of date to search by", default="modified"),
    ] = "modified",
    days_ago: Annotated[
        int,
        Field(
    ...
    operator: Annotated[
        ...
        Field(description="Search operator for date matching", default="within"),
    ] = "within",
    ctx=None,
):
    """
    ...
    try:
        ...
        raise McpError(
            ErrorData(
                code=INTERNAL_ERROR, message=f"Failed to search by date: {str(e)}"
            )
        )
...
async def list_notes_tool(
    directory: Optional[str] = None, recursive: bool = True, ctx=None
):
    """
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to list notes: {str(e)}")
        )
...
async def list_folders_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to list folders: {str(e)}")
        )


@mcp.tool()
async def move_note_tool(
    source_path: str, destination_path: str, update_links: bool = True, ctx=None
):
    """
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to move note: {str(e)}")
        )
...
async def create_folder_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to create folder: {str(e)}")
        )
...
async def move_folder_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to move folder: {str(e)}")
        )
...
async def add_tags_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to add tags: {str(e)}")
        )
...
async def update_tags_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to update tags: {str(e)}")
        )
...
async def remove_tags_tool(path: str, tags: list[str], ctx=None):
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to remove tags: {str(e)}")
        )
...
async def get_note_info_tool(path: str, ctx=None):
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to get note info: {str(e)}")
        )
...
async def get_backlinks_tool(
    ...
    include_context: Annotated[
        bool,
        Field(description="Whether to include text context around links", default=True),
    ] = True,
    context_length: Annotated[
        int,
        Field(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to get backlinks: {str(e)}")
        )
...
async def get_outgoing_links_tool(
    ...
    check_validity: Annotated[
        bool, Field(description="Whether to check if linked notes exist", default=False)
    ] = False,
    ctx=None,
):
    """
    ...
    try:
        ...
        raise McpError(
            ErrorData(
                code=INTERNAL_ERROR, message=f"Failed to get outgoing links: {str(e)}"
            )
        )
...
async def find_broken_links_tool(
    ...
    try:
        ...
        raise McpError(
            ErrorData(
                code=INTERNAL_ERROR, message=f"Failed to find broken links: {str(e)}"
            )
        )
...
async def list_tags_tool(
    include_counts: Annotated[
        bool,
        Field(description="Whether to include usage count for each tag", default=True),
    ] = True,
    sort_by: Annotated[
        Literal["name", "count"],
        Field(
    ...
    try:
        ...
        raise McpError(
            ErrorData(code=INTERNAL_ERROR, message=f"Failed to list tags: {str(e)}")
        )
